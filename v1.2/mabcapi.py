...
def start(action, pwd1, pwd2, BL, BD, phrase):
    if action not in ['e', 'd', 'fe', 'fd']:
        print("Wrong action!")
        print("Use e/d")
        usage()
    if not (pwd1.islower() and pwd2.islower()):
    ...
    if action == 'e':
        alphabet = 'abcdefghijklmnopqrstuwvxyz1234567890`~!@#$%^&*()-_=+[{]};:\'\",<.>/?'
    ...
    if action == 'fd':
        alphabet = 'abcdefghijklmnopqrstuwvxyz1234567890`~!@#$%^&*()-_=+[{]};:\'\",<.>/?'
        ciphertext = extract_cipher_text_from_file(phrase)
        num_to_delete = len(pwd2) - BL
        total_char = len(pwd2) - num_to_delete - 1
        mod = modify_alphabet(pwd1, alphabet)
...
def usage():
    ...
    print("")
    print("e/d/fe/fd - encrypt / decrypt / encrypt-from-file / decrypt-from-file")
...
def extract_plaintext_from_file(filename):
    try:
...
def extract_cipher_text_from_file(filename):
    try:
        with open(filename, 'r') as file:
            return file.readlines()
    except FileNotFoundError:
